BasicGame.BLEAK_HOUSE = function (game) {
...
BasicGame.BLEAK_HOUSE.prototype = Object.create(BasicGame.Breakout.prototype);


BasicGame.BLEAK_HOUSE.prototype.create = function () {

  BasicGame.Breakout.prototype.create.call(this);

  this.currentStateName = 'BLEAK_HOUSE';
  ...
  this.bleakSize = 14;
  this.bleakIndex = 0;
  this.bleakFinished = false;
  this.BLEAK_TEXTS = [];
  startY = this.game.height;
  var maxOnScreenTexts = Math.floor(this.game.height / this.bleakSize) + 1;
  for (var i = 0; i < maxOnScreenTexts; i++) {
    this.BLEAK_TEXTS.push(this.game.add.text(0, startY + i*this.bleakSize*1.5, BLEAK_HOUSE[this.bleakIndex], { fill: "#ffffff", font: this.bleakSize +"pt Arial" }));
    this.BLEAK_TEXTS[i].tint = 0x888888;
    this.BLEAK_TEXTS[i].index = i;
    this.bleakIndex++;
  }
};


BasicGame.BLEAK_HOUSE.prototype.update = function () {

  BasicGame.Breakout.prototype.update.call(this);

  if (this.bleakFinished) return;

  this.BLEAK_TEXTS.forEach(function (text) {

    text.y -= 1;

    if (text.y + text.height < 0) {
      if (text.index == 0) text.y = this.BLEAK_TEXTS[this.BLEAK_TEXTS.length - 1].y + this.bleakSize*1.5;
      else text.y = this.BLEAK_TEXTS[text.index - 1].y + this.bleakSize*1.5;
      // text.y = this.game.height;
      text.text = BLEAK_HOUSE[this.bleakIndex];
      this.bleakIndex++;

      if (this.bleakIndex == BLEAK_HOUSE.length) {
        this.bleakFinished = true;
      }
    }
...
BasicGame.BLEAK_HOUSE.prototype.restartBall = function (ball,paddle) {
...
BasicGame.BLEAK_HOUSE.prototype.shutdown = function () {
...
BasicGame.BLEAK_HOUSE.prototype.constructor = BasicGame.Breakout;
...
"",
"","","","","","","","","",
"To read more of Bleak House by Charles Dickens,",
